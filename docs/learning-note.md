...
▼ 学んだこと
    ...
    ②　justify-content と align-items の違い（重要）
        flex-direction によって「軸」が変わる
    ...
    ③　flex は「display: flex」が前提
        ・flex-direction だけ書いても効かない
        ・必ず display: flex が必要

    ④　CSSが効かない原因の多くは「クラス名ミス」
        ・「.input」と「.inputs」
        ・「.button」と「.buttons」
        ☞完全一致しないと適用されない
...
▼ つまづいたポイント
    ・justify-content で横中央になると思っていた
    ・flex-direction: column のときに軸が変わることを理解していなかった
    ・クラス名のミスに気づくのに時間がかかった

▼ 解決方法
    ・軸の考え方を整理した
    ・align-items を使うことで横中央にできた
    ・クラス名をHTMLとCSSで一致させた

▼ 理解したこと
    ・CSSは「見た目」ではなく「構造（レイアウト）」を作っている
    ・flex は「並べるルール」を決めるもの
    ・レスポンシブは「画面ごとに最適なUIに切り替えること」

▼ 今後に活かすこと
    ・レイアウトが崩れた場合は、まず「flexの軸」を疑う
    ・CSSが効かないときは「セレクタ」を確認する
    ・スマホで使う前提でUIを考える
